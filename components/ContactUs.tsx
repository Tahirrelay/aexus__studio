...
//     // Form element ko pehle hi variable mein save kar lein
//     const formElement = event.currentTarget;
//     const data = new FormData(formElement);
//     data.append('interest', selectedInterest);

//     startTransition(async () => {
//       const res = await submitContactForm(null, data);
//       setFormResponse(res);
//       if (res.success) {
//         formElement.reset(); // Yahan save kiye gaye variable ko use karein
//       }
//     });
//   };

//   return (
//     <section className="relative w-full bg-[#0A1B31] py-12 sm:py-16 px-4 sm:px-6 lg:px-8 select-none overflow-hidden">
//       <div className="absolute inset-0 pointer-events-none" style={{ background: '#0A1B31' }} />
...
//           <div className="hidden lg:block relative w-full h-[360px] rounded-3xl overflow-hidden border border-white/10 bg-[#0a0f18] shadow-[0_30px_70px_rgba(0,0,0,0.9)] backdrop-blur-xl group">
//             <Image 
//               src="/contact-graphic.png" 
//               alt="3D Stone and Glowing Cube Graphic"
//               fill
...
//             <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5 mb-4 sm:gap-5">
//               <div>
...
//             >
//               <span>{isPending ? 'SENDING...' : 'GET UPDATES'}</span>
//             </button>
...
export default function ContactUs() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      // Yahan apni EmailJS ki credentials dalein
      await emailjs.sendForm(
        'service_zeoj3c3',   // Apna Service ID yahan dalein
        'template_529xuag',  // Apna Template ID yahan dalein
        formRef.current,
        'MDnydqlaC45KNly0_'    // Apna Public Key yahan dalein
      );
      ...
      setFormResponse({ success: false, message: 'Something went wrong. Please try again.' });
    } finally {
      setLoading(false);
    }
  };
