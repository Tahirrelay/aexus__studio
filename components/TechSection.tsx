...
function TechCard({ item }: { item: typeof techStackData[0] }) {
  ...
  return (
    <div
      ...
      className="group relative flex flex-col items-center justify-center p-4 sm:p-5 h-[130px] sm:h-[155px] rounded-[24px] bg-[#0c0e15] border border-white/20 cursor-pointer overflow-hidden transition-all duration-500 hover:border-orange-500 text-center"
      style={{
        boxShadow: `
    ...
    >
      ...
      <span className={`absolute top-3 right-4 text-[10px] font-mono tracking-widest z-15 transition-colors ${isHovered ? 'text-black/70 font-semibold' : 'text-white/40'}`}>
        {item.id}
      </span>
          ...
          <div className="lg:col-span-7 order-1 lg:order-2 flex flex-col gap-6">
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {techStackData.map((item, index) => (
                <div key={item.id} className={`${!showAll && index >= 6 ? 'hidden lg:flex' : 'flex'} flex-col`}>
                  <TechCard item={item} />
                </div>
              ))}
            </div>
            <div className="flex justify-center mt-2 lg:hidden">
              <button
                ...
                className="px-8 py-3.5 rounded-xl bg-[#0c0e15] border border-white/20 text-white font-bold text-xs uppercase tracking-wider shadow-[0_6px_20px_rgba(0,0,0,0.6),inset_0_1px_1px_rgba(255,255,255,0.15)] hover:bg-orange-500 hover:text-black hover:border-orange-500 transition-all duration-300 active:scale-95"
              >
