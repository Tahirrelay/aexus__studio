...
export default function FloatingAiContact() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Form submit hone ka token set karein
      sessionStorage.setItem('form_token', Date.now().toString());

      setLoading(false);
      setFormResponse({ success: true, message: 'Thank you! Your message has been sent successfully.' });
      
      // 1.5 seconds ke baad Thank You page par le jayein
      setTimeout(() => {
        setIsOpen(false);
        setFormResponse(null);
            ...
            <div>
              <div className="flex items-center justify-between mb-6 pb-4 border-b border-white/10">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-white text-base font-black uppercase tracking-wider">AEXUS STUDIOS</h3>
                    ...
                    return (
                      <button
                        ...
                        className={`py-2.5 px-2 rounded-xl font-black text-[10px] uppercase tracking-wider transition-all cursor-pointer text-center ${
                          isSelected ? 'bg-[#ff8800] text-black shadow-md' : 'bg-black/40 border border-white/10 text-white/70 hover:text-white'
                        }`}
                      >
                        {label}
                      </button>
